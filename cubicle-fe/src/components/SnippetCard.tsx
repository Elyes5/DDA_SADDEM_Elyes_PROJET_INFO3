...
export const SnippetCard: React.FC<SnippetCardProps> = ({ snippet }) => {
  ...
  const handleSubmitReview = (): void => {
    ...
    void dispatch(addOrUpdateReview({
    ...
    <>
      ...
      >
        <CardHeader
          ...
          action={
            <Stack direction="row" spacing={1} alignItems="center" mt={1} mr={1}>
              {isOwner && (
                <Stack direction="row" spacing={0.5}>
                  <Tooltip title="Modifier">
                    <IconButton size="small" onClick={() => setIsEditModalOpen(true)} color="primary">
                      <EditIcon fontSize="small" />
                    </IconButton>
                  </Tooltip>
                  <Tooltip title="Supprimer">
                    <IconButton size="small" onClick={() => setIsDeleteModalOpen(true)} color="error">
              ...
              {snippet.topic && (
                <Chip label={snippet.topic.name} size="small" color="primary" sx={{ fontWeight: 700, fontSize: '0.65rem' }} />
              )}
              <Chip label={snippet.language ?? 'Text'} size="small" variant="outlined" sx={{ fontWeight: 700, fontSize: '0.65rem' }} />
            </Stack>
          }
        />

        <CardContent sx={{ pt: 0 }}>
          <Typography variant="h6" fontWeight={800} mb={1}>{snippet.title}</Typography>
          <Typography variant="body2" color="text.secondary" mb={2}>{snippet.description}</Typography>
          
          <Box sx={{ position: 'relative', borderRadius: 2, overflow: 'hidden' }}>
            <IconButton 
              size="small" 
              onClick={handleCopyCode}
              sx={{ position: 'absolute', right: 8, top: 8, zIndex: 2, color: 'white', bgcolor: 'rgba(255,255,255,0.2)', '&:hover': { bgcolor: 'rgba(255,255,255,0.3)' } }}
            >
            ...
            <SyntaxHighlighter
              ...
              style={vscDarkPlus}
              customStyle={{ padding: '20px', margin: 0, fontSize: '0.8rem', backgroundColor: '#1e1e1e' }}
            >
        ...
        <CardActions sx={{ px: 2, py: 1, justifyContent: 'space-between', bgcolor: 'grey.50' }}>
          <Stack direction="row" spacing={2}>
            <Box display="flex" alignItems="center">
              <IconButton size="small" onClick={handleLike} color={isLiked ? 'error' : 'default'}>
                {isLiked ? <Favorite fontSize="small" /> : <FavoriteBorder fontSize="small" />}
              </IconButton>
              <Typography variant="caption" fontWeight={700}>{snippet.like_count}</Typography>
            </Box>
            ...
            <Button
              ...
              sx={{ textTransform: 'none', fontWeight: 700, color: expanded ? 'primary.main' : 'text.secondary' }}
            >
        ...
        <Collapse in={expanded} timeout="auto" unmountOnExit>
          ...
          <Box p={2}>
            {!isOwner && user ? (
              <>
                <Typography variant="subtitle2" fontWeight={800} mb={1}>Laisser une review</Typography>
                    ...
                    InputProps={{
                      endAdornment: (
                        <InputAdornment position="end">
                          <IconButton onClick={handleSubmitReview} disabled={!reviewText.trim()} color="primary">
                            <Send fontSize="small" />
                          </IconButton>
                        </InputAdornment>
                      ),
            ...
            <Stack spacing={2}>
              {snippet.reviews && snippet.reviews.length > 0 ? (
                snippet.reviews.map((rev) => (
                  <Box key={rev.id} sx={{ display: 'flex', gap: 2 }}>
                    <Avatar 
                      sx={{ width: 32, height: 32, fontSize: '0.8rem', cursor: 'pointer' }}
                    ...
                    <Box sx={{ flexGrow: 1 }}>
                      <Stack direction="row" spacing={1} alignItems="center">
                        ...
                        <Rating value={rev.rating} readOnly size="small" sx={{ fontSize: '0.75rem' }} />
                      </Stack>
                ...
                <Typography variant="caption" color="text.secondary">Aucune review pour le moment.</Typography>
              )}
            </Stack>
          </Box>
        </Collapse>
      ...
      >
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1, fontWeight: 800 }}>
          <WarningAmberRounded color="error" /> Confirmer la suppression
        </DialogTitle>
        <DialogContent>
          <DialogContentText>
            Êtes-vous sûr de vouloir supprimer le snippet <strong>"{snippet.title}"</strong> ? 
            Cette action est irréversible.
          </DialogContentText>
        </DialogContent>
        <DialogActions sx={{ p: 2 }}>
          <Button onClick={() => setIsDeleteModalOpen(false)} color="inherit">Annuler</Button>
